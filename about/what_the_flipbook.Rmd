...
Visual superimposition of new code on previous code and new output on previous output yields a ["blink comparator"](https://en.wikipedia.org/wiki/Blink_comparator){target="_blank"} type experience, and incrementalism assists viewers in linking specific components of code with resultant behavior.  I believe that flipbooks are very promising as communication, educational, and reference medium.

Click on the example below. Then you should be able to "flip" (left and right arrows) through the book to see how to make an animated racing bar chart of at population size in the most populous countries over time.   
...
In mid 2018 Garrick Aden-Buie, a data scientist at the Moffitt Cancer Center, created and debuted a side-by-side format for code and figure output in the slide-show format, Xaringan, an RMarkdown tool.  He demonstrated in incremental steps how someone might build up a plot using the popular R Stats package ggplot2.  In August 2018, I emulated this work, but used an entirely sequential and more incremental work flow that I'd been working on and had termed "slow ggplot".  In September, Emi Tanaka, a Lecturer in Statistics at the University of Sydney, implemented a similar sequential workflow demonstration, that was partially automated.  After I approached Garrick and Emi in October about working together to create a "flipbook" of exemplary data visualizations, we soon had a fully automated framework to parse code, and present its reconstruction along with associated output!  The code designed to parse and reconstruct code was applied to a collection of plots that I created over about about a year and a half in learning ggplot.  After several months of preparation, including rewriting code in a "slow" ggplot style, and providing written description to introduce the plots, I publicized the work in February, 2019; the flipbook was met the an enthusiastic reception.  Since then, a number changes have been made to the foundational code to improve the tool and to enhance its usefulness. 
...
### Theortical description

I'm currently working on a manuscript thinking about flipbooks from a theoretical perspective including a discussion of human cognition, describing the format and our particular implementation, as well as articulating its merits. 

### Extention to non sequential code
